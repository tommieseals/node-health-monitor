...
class SSHCollector(BaseCollector):
    ...
    def _get_client(self) -> paramiko.SSHClient:
        ...
        # Host keys must be known in advance. We deliberately do NOT use
        # AutoAddPolicy: silently trusting an unverified host key would make
        # every connection vulnerable to man-in-the-middle attacks. Load the
        # user's known_hosts (plus an optional per-node file) and reject
        # anything unknown. See README "SSH host key verification".
        client.load_system_host_keys()
        if ssh_config.known_hosts:
            known_hosts_path = Path(ssh_config.known_hosts).expanduser()
    ...
    def collect(self) -> NodeHealth:
        ...
        try:
            # Establish the SSH connection first so a connection failure marks
            # the node unreachable instead of silently producing zeroed
            # metrics (execute_command catches per-command errors).
            self._get_client()
