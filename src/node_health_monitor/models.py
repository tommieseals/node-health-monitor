...
class NodeHealth:
    ...
    def load_status(self) -> HealthStatus:
        # Use the absolute 1-minute load average. Thresholds, docs, and the
        # example configs all express load limits as absolute values
        # (e.g. warning at 4.0, critical at 8.0), so no CPU-count scaling here.
        return self._get_status(self.load_average[0], "load")
...
class ClusterHealth:
    ...
    def critical_count(self) -> int:
        bad = (HealthStatus.CRITICAL, HealthStatus.UNREACHABLE)
        return sum(1 for n in self.nodes if n.status in bad)
