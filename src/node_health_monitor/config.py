...
class Thresholds:
    ...
    load_warning: float = 4.0  # Absolute 1-minute load average
...
class SSHConfig:
    ...
    timeout: int = 10
    known_hosts: str | None = None  # Extra known_hosts file (besides ~/.ssh/known_hosts)
...
class NodeConfig:
    ...
    def from_dict(cls, name: str, data: dict[str, Any]) -> "NodeConfig":
        ...
        remediation = (
            RemediationConfig.from_dict(data["remediation"]) if "remediation" in data else None
        )
    ...
    def _to_dict(self) -> dict[str, Any]:
        ...
        for node in self.nodes:
            ...
            if node.ssh:
                ...
                if node.ssh.known_hosts:
                    node_data["ssh"]["known_hosts"] = node.ssh.known_hosts
            if node.services:
